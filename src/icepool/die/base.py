...
class BaseDie():
    ...
    * `icepool.align()` and the like are convenient for making dice share the
    ...
    def ndim(self):
        ...
        Otherwise, returns `'icepool.Scalar'` for a `ScalarDie` and
        `'icepool.Empty'` for an `EmptyDie`.
        """
    ...
    def _wrap_unpack(self, func):
        """Wraps `func` so that vector outcomes are unpacked into separate arguments. 
        
        Scalar outcomes are unchanged.
        """
        ...
        Args:
            outcomes: Selects which outcomes to reroll. Options:
                * A callable that takes an outcome and returns `True` if it
                    should be rerolled. The callable will be supplied with one
                    argument per `ndim` if this is a `VectorDie`.
        ...
        Args:
            repl: One of the following:
                ...
                * A callable mapping old outcomes to new outcomes.
                    The callable will be supplied with one argument per `ndim` 
                    if this is a `VectorDie`.
            ...
            ndim: Sets the `ndim` of the result. If not provided, `ndim` will be 
    ...
    def explode(self, outcomes=None, *, max_depth=None):
        ...
        Args:
            outcomes: Which outcomes to explode. Options:
                ...
                * A callable that takes an outcome and returns `True` if it 
                    should be exploded. The callable will be supplied with one 
                    argument per `ndim` if this is a `VectorDie`.
