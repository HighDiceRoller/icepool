...
class AdjustIntCountGenerator(OutcomeCountGenerator):
    """A meta-generator that adjusts int counts."""

    def __init__(self, src: OutcomeCountGenerator, /, func: Callable[[Any, int],
                                                                     int]):
        """
        Args:
            src: The generator to be wrapped.
            func: A function that takes in an outcome and a count of type `int`,
                and returns a new count of type `int`.
        """
        self._src = src
        self._func = func

    def outcomes(self) -> Sequence:
        return self._src.outcomes()
    ...
    def _generate_min(self, min_outcome) -> NextOutcomeCountGenerator:
        for popped_src, counts, weight in self._src._generate_min(min_outcome):
            yield AdjustIntCountGenerator(popped_src, self._func), tuple(
                self._func(min_outcome, count) for count in counts), weight

    def _generate_max(self, max_outcome) -> NextOutcomeCountGenerator:
        for popped_src, counts, weight in self._src._generate_max(max_outcome):
            yield AdjustIntCountGenerator(popped_src, self._func), tuple(
                self._func(max_outcome, count) for count in counts), weight
    ...
    def _key_tuple(self) -> tuple:
        return AdjustIntCountGenerator, self._src, self._func

    def __eq__(self, other) -> bool:
        if not isinstance(other, AdjustIntCountGenerator):
            return False
