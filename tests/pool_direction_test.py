...
def test_auto_direction_uniform():
    algorithm, direction = icepool.evaluate_sum._select_algorithm(icepool.d6.pool()[0,0,1,1])
    assert direction > 0
    algorithm, direction = icepool.evaluate_sum._select_algorithm(icepool.d6.pool()[1,1,0,0])
    assert direction < 0

# Above that, the auto direction should favor the wide-to-narrow ordering.
def test_auto_direction_max_truncate_min():
    algorithm, direction = icepool.evaluate_sum._select_algorithm(Pool([d8, d6, d6, d6])[1,1,1,0])
    assert direction > 0
    algorithm, direction = icepool.evaluate_sum._select_algorithm(Pool([d8, d6, d6, d6])[0,1,1,1])
    assert direction > 0
