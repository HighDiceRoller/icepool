...
class VerifyDirection(icepool.OutcomeCountEval):
...
def test_auto_direction_uniform():
    algorithm, direction = icepool.sum_gen._select_algorithm(icepool.d6.pool()[0,0,1,1])
    assert direction > 0
    algorithm, direction = icepool.sum_gen._select_algorithm(icepool.d6.pool()[1,1,0,0])
    assert direction < 0
