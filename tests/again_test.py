...
from icepool import AgainExpression, d6, Die, Reroll
...
def test_again_evaluate_plus_d6():
    x = AgainExpression() + d6
    assert x._evaluate(d6) == 2 @ d6


def test_again_evaluate_d6_plus():
    x = d6 + AgainExpression()
    assert x._evaluate(d6) == 2 @ d6


def test_again_evaluate_plus_again():
    x = AgainExpression() + AgainExpression()
    assert x._evaluate(d6) == 2 @ d6


def test_explode_d6_depth_0():
    die = Die([1, 2, 3, 4, 5, 6 + AgainExpression()], again_depth=0)
    assert die == d6


def test_explode_d6_depth_1():
    die = Die([1, 2, 3, 4, 5, 6 + AgainExpression()], again_depth=1)
    assert die == Die({
        1: 6,
        2: 6,
        3: 6,
...
def test_again_plus_again_depth_0():
    die = Die(
        [1, 2, 3, 4, 5, AgainExpression() + AgainExpression()],
        again_depth=0,
        again_end=3)
...
def test_again_plus_again_depth_1():
    die = Die(
        [1, 2, 3, 4, 5, AgainExpression() + AgainExpression()], again_end=3)
    assert die == Die([1, 2, 3, 4, 5, 2 @ d6])


def test_again_reroll():
    die = Die([1, 2, 3, 4, 5, AgainExpression()],
              again_depth=0,
              again_end=Reroll)
...
def test_owod():
    result = Die({0: 7, 1: 2, AgainExpression() + 1: 1}, again_depth=3)
