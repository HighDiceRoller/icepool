...
def test_align_range_symmetric_difference():
    ...
    assert a.equals(icepool.Die(range(1, 8), weights=[1, 1, 1, 1, 0, 0, 0]))
    assert b.equals(icepool.Die(range(1, 8), weights=[0, 1, 1, 1, 1, 1, 1]))

def test_align_range_subset():
    a, b = icepool.align_range(icepool.d4+1, icepool.d8)
    assert a.equals(icepool.Die(range(1, 9), weights=[0, 1, 1, 1, 1, 0, 0, 0]))
    assert b.equals(icepool.Die(range(1, 9), weights=[1, 1, 1, 1, 1, 1, 1, 1]))
