...
def test_map_star():
    a = icepool.Die([(0, 0)]).map(lambda x, y: (x, y), repeat=1, star=1)
    b = icepool.Die([(0, 0)]).map(lambda x, y: (x, y), repeat=2, star=1)
    c = icepool.Die([(0, 0)]).map(lambda x, y: (x, y), repeat=None, star=1)
    assert a == b
    assert b == c
